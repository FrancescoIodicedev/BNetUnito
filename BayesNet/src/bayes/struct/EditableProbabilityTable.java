 ...
 */
public class EditableProbabilityTable implements CategoricalDistribution,CustomFactor {
    ...
    public EditableProbabilityTable(Collection<RandomVariable> vars) {
        this(vars.toArray(new RandomVariable[vars.size()]));
    }

    public EditableProbabilityTable(RandomVariable... vars) {
        this(new double[ProbUtil.expectedSizeOfProbabilityTable(vars)], vars);
    }

    public EditableProbabilityTable(double[] vals, RandomVariable... vars) {
        if (null == vals) {
    ...
    @Override
    public EditableProbabilityTable normalize() {
        double s = getSum();
        if (s != 0 && s != 1.0) {
            for (int i = 0; i < values.length; i++) {
                values[i] = values[i] / s;
    ...
    public CategoricalDistribution multiplyBy(CategoricalDistribution multiplier) {
        return pointwiseProduct((EditableProbabilityTable) multiplier);
    }

    @Override
    public CategoricalDistribution multiplyByPOS(
            CategoricalDistribution multiplier, RandomVariable... prodVarOrder) {
        return pointwiseProductPOS((EditableProbabilityTable) multiplier, prodVarOrder);
    }
    ...
    public Set<RandomVariable> getArgumentVariables() {
     ...
     public EditableProbabilityTable sumOut(RandomVariable... vars) {
        Set<RandomVariable> soutVars = new LinkedHashSet<RandomVariable>(
                this.randomVarInfo.keySet());
        for (RandomVariable rv : vars) {
            soutVars.remove(rv);
        }
        final EditableProbabilityTable summedOut = new EditableProbabilityTable(soutVars);
    ...
    public Factor pointwiseProduct(Factor multiplier) {
        return pointwiseProduct((Factor) multiplier);
    }

    public Factor pointwiseProductPOS(Factor multiplier,
                                      RandomVariable... prodVarOrder) {
        return pointwiseProductPOS((Factor) multiplier, prodVarOrder);
    }
    ...
    public EditableProbabilityTable divideBy(EditableProbabilityTable divisor) {
        if (!randomVarInfo.keySet().containsAll(divisor.randomVarInfo.keySet())) {
            throw new IllegalArgumentException(
                    "Divisor must be a subset of the dividend.");
        }

        final EditableProbabilityTable quotient = new EditableProbabilityTable(randomVarInfo
                .keySet());
        ...
        if (1 == divisor.getValues().length) {
            ...
            divisor.iterateOverTable((Iterator) divisorIterator);
        }

        return quotient;
    }

    public EditableProbabilityTable pointwiseProduct(final EditableProbabilityTable multiplier) {
    ...
    public EditableProbabilityTable pointwiseProductPOS(
            final EditableProbabilityTable multiplier, RandomVariable... prodVarOrder) {
        final EditableProbabilityTable product = new EditableProbabilityTable(prodVarOrder);
        ...
        if (1 == product.getValues().length) {
            ...
            ProbabilityTable.Iterator di = new ProbabilityTable.Iterator() {
                ...
                public void iterate(Map<RandomVariable, Object> possibleWorld,
                                    double probability) {
                    int term1Idx = termIdx(term1Values, EditableProbabilityTable.this,
                            possibleWorld);
                ...
                private int termIdx(Object[] termValues, EditableProbabilityTable d,
            ...
            product.iterateOverTable((Iterator) di);
        }
